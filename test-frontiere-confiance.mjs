...
console.log("\n=== 10. Sur la base RÉELLE : plus aucun verdict catégorique ===");
{
  ...
  /* MOUVEMENT NOMMÉ (09/09/2026, import strict lot 5 — 18 citations de plus, 92 en tout) : 58 → 73 sous conditions, 14 → 17 refusés (Malaysia, China Eastern, Air Mauritius cabines), 230 → 212 à confirmer ; quatre lignes non
  ...
  /* MOUVEMENT NOMMÉ (12/09/2026, SAS soute — page nationale suédoise citée) : 143 → 144
     sous conditions, 126 → 125 à confirmer ; `legacy_unreviewed` 111 → 110. */
  /* MOUVEMENT NOMMÉ (12/09/2026, lot de 30 compagnies) : onze décisions gagnent une phrase
     officielle ; 7 deviennent sous conditions et 4 refusées. 189 politiques sont citées,
     dont 188 décisives ; Virgin Australia cabine reste la seule citée `case_by_case`. */
  /* MOUVEMENT NOMMÉ (13/09/2026, vague de 31 dossiers) : 203 politiques sont citées,
     dont 199 décisives ; China Southern cabine, South African soute et Volotea soute rejoignent
     Virgin Australia cabine parmi les quatre `case_by_case`. */
  ...
  const CITEES_V3 = [
    /* MOUVEMENT NOMMÉ (09/09/2026, import strict lot 5 — 18 citations de plus, 92 en tout) : 74 → 92, nominativement. */
    ...
    /* Lot 6 (09/09/2026) : 21 citations de plus, 113 en tout. Air China cabine REFUSÉE (la fiche dit `not_offered`). */
    ...
    /* MOUVEMENT NOMMÉ (13/09/2026, vague de 31 dossiers) : quatorze canaux supplémentaires
       reçoivent une citation officielle ; quatre restent volontairement non décisifs. */
    ...
    /* MOUVEMENT NOMMÉ (13/09/2026, fret officiel des principales compagnies) : vingt et un
       canaux fret entrent nominativement ; les quatre `case_by_case` restent non décisifs. */
  ...
  check("et 240 d'elles sont des décisions (onze case_by_case restent prudentes)",
    decideesCitees.length === 240 && decideesCitees.includes("airline_air_france.cabin") && decideesCitees.includes("airline_bangkok_airways.cargo") && decideesCitees.includes("airline_sas.hold")
      ...
      && !decideesCitees.includes("airline_volotea.hold")
...
console.log("\n=== 13 bis. Le verdict dérivé, et ce qui ne revient JAMAIS avec lui ===");
{
  ...
  check("les TROIS canaux refusés — et eux seuls — rendent « aucun canal accepté »",
    verdictDeFiche({ cabin: P("denied"), hold: P("denied"), cargo: P("denied") }).cle === "premium.verdict_none");
  /* Sur la base RÉELLE : les canaux absents du dépôt ne doivent transformer aucune fiche en
     refus total. British Airways garde son refus cabine, mais son fret IAG Cargo désormais cité
     rend la fiche positive sous conditions. */
  {
    const kbR = loadKB();
    ...
    check("British Airways : cabine refusée sur preuve, fret cité, FICHE ouverte sous conditions",
      ba?.premium?.policy?.cabin?.status === "denied"
        && ba?.premium?.policy?.cargo?.status === "accepted_with_conditions"
        && verdictDeFiche(ba?.premium?.policy).cle === "premium.verdict_open_conditions",
...
console.log("\n=== 13 quater. LA FRONTIÈRE S'APPLIQUE AUSSI À LA CHALEUR ===");
{
  ...
  check("état figé : 7 règles summer_embargo, 2 citées et 5 officielles non citées",
    embargos.length === 7
      && embargos.filter((r) => niveauDePreuveRegle(r) === "citee").map((r) => r.id).sort().join() === ["rule_ac_summer_embargo", "rule_american_cargo_heat_official_2026_09_12"].join()
      && embargos.filter((r) => niveauDePreuveRegle(r) === "officielle_non_citee").length === 5,
