  ...
  if (existsSync(APRES) && existsSync(APPROUVE)) {
    ...
    check("l'AVANT du lot 8 EST l'après du lot 7 — chaîne continue",
      readFileSync("test-baselines/import-strict-lot-8-avant.json", "utf8")
        === readFileSync("test-baselines/import-strict-lot-7-apres.json", "utf8"));
    check("l'AVANT du lot 7 EST l'après du lot 6 — chaîne continue",
      readFileSync("test-baselines/import-strict-lot-7-avant.json", "utf8")
        === readFileSync("test-baselines/import-strict-lot-6-apres.json", "utf8"));
    check("l'AVANT du lot 6 EST l'après du lot 5 — chaîne continue",
      readFileSync("test-baselines/import-strict-lot-6-avant.json", "utf8")
    ...
    check("l'AVANT du lot 4 EST l'après des lots 2 et 3 — chaîne continue",
      readFileSync("test-baselines/import-strict-lot-4-avant.json", "utf8")
        === readFileSync("test-baselines/import-strict-lots-2-3-apres.json", "utf8"));
    check("l'AVANT des lots 2 et 3 EST l'après de l'import V3 — chaîne continue",
      readFileSync("test-baselines/import-strict-lots-2-3-avant.json", "utf8")
...
console.log("=== Preuve PERMANENTE Import strict lots 2 et 3 — 24 citations, et rien d'autre ne bouge (baselines FIGÉES) ===");
{
  /* Même méthode que la preuve V3 : cartes appariées par compagnie. Entre l'après de l'import V3
   * et l'après des lots 2 et 3 : 208 cartes sur 1 560, douze compagnies importées et elles seules ;
   * 248 canaux « à confirmer » → accepté sous conditions, 32 → refusé (Cathay, EVA Air, ANA
   * cabines) ; aucun vers `allowed` ; AUCUN verdict ne bouge (52 « conditional », 20 « unknown »
   * — les scénarios de la baseline avaient déjà leurs canaux prouvés au lot précédent). */
  const AVANT = "test-baselines/import-strict-lots-2-3-avant.json";
  const APRES = "test-baselines/import-strict-lots-2-3-apres.json";
  const IMPORTEES = ["airline_air_canada", "airline_delta", "airline_jetblue", "airline_air_transat", "airline_air_europa",
    "airline_cathay_pacific", "airline_eva_air", "airline_air_india", "airline_avianca", "airline_ethiopian", "airline_ana", "airline_jal", "airline_etihad"];
  check("les deux baselines des lots 2 et 3 sont versionnées", existsSync(AVANT) && existsSync(APRES));
  if (existsSync(AVANT) && existsSync(APRES)) {
    const avant = JSON.parse(readFileSync(AVANT, "utf8")), apres = JSON.parse(readFileSync(APRES, "utf8"));
    const idDe = (s) => s.split(" | ")[0];
    const statutsDe = (s) => (s.split(" | ").find((seg) => seg.startsWith("st:")) ?? "st:?/?/?").slice(3).split("/");
    ...
    check("SEULES des compagnies importées aux lots 2 et 3 changent de carte — 12 d'entre elles",
      [...changees.keys()].every((id) => IMPORTEES.includes(id)) && changees.size === 12,
      [...changees.keys()].filter((id) => !IMPORTEES.includes(id)).join(", ") || `${changees.size} compagnies`);
    check("208 cartes sur 1 560 changent (appariées par compagnie)", cartes === 208 && total === 1560, `${cartes} / ${total}`);
    check("248 canaux « à confirmer » → accepté sous conditions, 32 → refusé, et RIEN d'autre",
      transitions.get("confirmation_required→accepted_with_conditions") === 248 && transitions.get("confirmation_required→denied") === 32 && transitions.size === 2,
      JSON.stringify([...transitions]));
...
console.log("=== Preuve PERMANENTE Import strict lot 4 — 22 citations, et rien d'autre ne bouge (baselines FIGÉES) ===");
{
  /* Même méthode. Entre l'après des lots 2 et 3 et l'après du lot 4 : 320 cartes sur 1 560, huit
   * compagnies importées et elles seules (Qantas et Alaska ne desservent aucun des 72 scénarios) ;
   * 372 canaux « à confirmer » → accepté sous conditions, 72 → refusé (Emirates, Aer Lingus cabines
   * pour tout chien ; Austrian, SWISS, Brussels cabines au-dessus de 8 kg) ; aucun vers `allowed` ;
   * AUCUN verdict ne bouge (52 « conditional », 20 « unknown »). Aer Lingus soute, REFUSÉE par
   * l'importeur, ne change pas : toujours « à confirmer ». */
  const AVANT = "test-baselines/import-strict-lot-4-avant.json";
  const APRES = "test-baselines/import-strict-lot-4-apres.json";
  const IMPORTEES = ["airline_austrian", "airline_swiss", "airline_ita_airways", "airline_emirates", "airline_american",
    "airline_brussels", "airline_westjet", "airline_aer_lingus"];
  check("les deux baselines du lot 4 sont versionnées", existsSync(AVANT) && existsSync(APRES));
  if (existsSync(AVANT) && existsSync(APRES)) {
    const avant = JSON.parse(readFileSync(AVANT, "utf8")), apres = JSON.parse(readFileSync(APRES, "utf8"));
    const idDe = (s) => s.split(" | ")[0];
    const statutsDe = (s) => (s.split(" | ").find((seg) => seg.startsWith("st:")) ?? "st:?/?/?").slice(3).split("/");
    ...
    check("SEULES des compagnies importées au lot 4 changent de carte — 8 d'entre elles",
      [...changees.keys()].every((id) => IMPORTEES.includes(id)) && changees.size === 8,
      [...changees.keys()].filter((id) => !IMPORTEES.includes(id)).join(", ") || `${changees.size} compagnies`);
    check("320 cartes sur 1 560 changent (appariées par compagnie)", cartes === 320 && total === 1560, `${cartes} / ${total}`);
    check("372 canaux « à confirmer » → accepté sous conditions, 72 → refusé, et RIEN d'autre",
      transitions.get("confirmation_required→accepted_with_conditions") === 372 && transitions.get("confirmation_required→denied") === 72 && transitions.size === 2,
      JSON.stringify([...transitions]));
    check("AUCUN canal ne va vers `allowed`, AUCUN verdict ne bouge (52 conditional, 20 unknown)",
      [...transitions.keys()].every((t) => !t.endsWith("→allowed")) && verdicts.get("conditional→conditional") === 52 && verdicts.get("unknown→unknown") === 20 && verdicts.size === 2,
      JSON.stringify([...verdicts]));
...
console.log("=== Preuve PERMANENTE Import strict lot 6 — 21 citations, et rien d'autre ne bouge (baselines FIGÉES) ===");
{
  /* Même méthode. Entre l'après du lot 5 et l'après du lot 6 : 136 cartes sur 1 560, neuf compagnies
   * importées et elles seules (South African Airways ne dessert aucun des 72 scénarios) ; 116 canaux
   * « à confirmer » → accepté sous conditions, 88 → refusé (Saudia, Kenya, Gulf Air cabines pour tout
   * chien, Kenya et Gulf Air soutes ; Aeromexico cabine au-dessus de 9 kg, EgyptAir cabine au-dessus
   * de 8 kg) ; aucun vers `allowed` ; AUCUN verdict ne bouge. */
  const AVANT = "test-baselines/import-strict-lot-6-avant.json";
  const APRES = "test-baselines/import-strict-lot-6-apres.json";
  const IMPORTEES = ["airline_royal_jordanian", "airline_saudia", "airline_air_china", "airline_gulf_air", "airline_kenya_airways", "airline_egyptair", "airline_united", "airline_aeromexico", "airline_latam"];
  check("les deux baselines du lot 6 sont versionnées", existsSync(AVANT) && existsSync(APRES));
  if (existsSync(AVANT) && existsSync(APRES)) {
    const avant = JSON.parse(readFileSync(AVANT, "utf8")), apres = JSON.parse(readFileSync(APRES, "utf8"));
    const idDe = (s) => s.split(" | ")[0];
    const statutsDe = (s) => (s.split(" | ").find((seg) => seg.startsWith("st:")) ?? "st:?/?/?").slice(3).split("/");
    ...
    check("SEULES des compagnies importées au lot 6 changent de carte — 9 d'entre elles",
      [...changees.keys()].every((id) => IMPORTEES.includes(id)) && changees.size === 9,
      [...changees.keys()].filter((id) => !IMPORTEES.includes(id)).join(", ") || `${changees.size} compagnies`);
    check("136 cartes sur 1 560 changent (appariées par compagnie)", cartes === 136 && total === 1560, `${cartes} / ${total}`);
    check("116 canaux « à confirmer » → accepté sous conditions, 88 → refusé, et RIEN d'autre",
      transitions.get("confirmation_required→accepted_with_conditions") === 116 && transitions.get("confirmation_required→denied") === 88 && transitions.size === 2,
      JSON.stringify([...transitions]));
    check("AUCUN canal ne va vers `allowed`, AUCUN verdict ne bouge (52 conditional, 20 unknown)",
      [...transitions.keys()].every((t) => !t.endsWith("→allowed")) && verdicts.get("conditional→conditional") === 52 && verdicts.get("unknown→unknown") === 20 && verdicts.size === 2,
      JSON.stringify([...verdicts]));
...
console.log("=== Preuve PERMANENTE Import strict lot 7 — 23 citations, et rien d'autre ne bouge (baselines FIGÉES) ===");
{
  /* Même méthode. Entre l'après du lot 6 et l'après du lot 7 : 16 cartes sur 1 560, deux compagnies importées et
   * elles seules (Aircalin et Air Algérie ; les huit autres ne desservent aucun des 72 scénarios) ;
   * 8 canaux « à confirmer » → accepté sous conditions, 16 → refusé (Aircalin cabine et soute pour
   * tout chien) ; aucun vers `allowed` ; AUCUN verdict ne bouge. */
  const AVANT = "test-baselines/import-strict-lot-7-avant.json";
  const APRES = "test-baselines/import-strict-lot-7-apres.json";
  const IMPORTEES = ["airline_aircalin", "airline_air_algerie"];
  check("les deux baselines du lot 7 sont versionnées", existsSync(AVANT) && existsSync(APRES));
  if (existsSync(AVANT) && existsSync(APRES)) {
    const avant = JSON.parse(readFileSync(AVANT, "utf8")), apres = JSON.parse(readFileSync(APRES, "utf8"));
    const idDe = (s) => s.split(" | ")[0];
    const statutsDe = (s) => (s.split(" | ").find((seg) => seg.startsWith("st:")) ?? "st:?/?/?").slice(3).split("/");
    ...
    check("SEULES des compagnies importées au lot 7 changent de carte — 2 d'entre elles",
      [...changees.keys()].every((id) => IMPORTEES.includes(id)) && changees.size === 2,
      [...changees.keys()].filter((id) => !IMPORTEES.includes(id)).join(", ") || `${changees.size} compagnies`);
    check("16 cartes sur 1 560 changent (appariées par compagnie)", cartes === 16 && total === 1560, `${cartes} / ${total}`);
    check("8 canaux « à confirmer » → accepté sous conditions, 16 → refusé, et RIEN d'autre",
      transitions.get("confirmation_required→accepted_with_conditions") === 8 && transitions.get("confirmation_required→denied") === 16 && transitions.size === 2,
      JSON.stringify([...transitions]));
    check("AUCUN canal ne va vers `allowed`, AUCUN verdict ne bouge (52 conditional, 20 unknown)",
      [...transitions.keys()].every((t) => !t.endsWith("→allowed")) && verdicts.get("conditional→conditional") === 52 && verdicts.get("unknown→unknown") === 20 && verdicts.size === 2,
      JSON.stringify([...verdicts]));
...
console.log("=== Preuve PERMANENTE Import strict lot 8 — 22 citations, et rien d'autre ne bouge (baselines FIGÉES) ===");
{
  /* Même méthode. Entre l'après du lot 7 et l'après du lot 8 : 48 cartes sur 1 560, quatre compagnies importées et
   * elles seules (Thai Airways, China Southern, SunExpress, Smartwings ; les six autres ne desservent
   * aucun des 72 scénarios) ; 40 canaux « à confirmer » → accepté sous conditions, 32 → refusé (Thai et
   * China Southern cabines pour tout chien, SunExpress cabine au-dessus de 8 kg) ; aucun vers
   * `allowed` ; AUCUN verdict ne bouge. */
  const AVANT = "test-baselines/import-strict-lot-8-avant.json";
  const APRES = "test-baselines/import-strict-lot-8-apres.json";
  const IMPORTEES = ["airline_thai_airways", "airline_china_southern", "airline_sunexpress", "airline_smartwings"];
  check("les deux baselines du lot 8 sont versionnées", existsSync(AVANT) && existsSync(APRES));
  if (existsSync(AVANT) && existsSync(APRES)) {
    const avant = JSON.parse(readFileSync(AVANT, "utf8")), apres = JSON.parse(readFileSync(APRES, "utf8"));
    const idDe = (s) => s.split(" | ")[0];
    const statutsDe = (s) => (s.split(" | ").find((seg) => seg.startsWith("st:")) ?? "st:?/?/?").slice(3).split("/");
    ...
    check("SEULES des compagnies importées au lot 8 changent de carte — 4 d'entre elles",
      [...changees.keys()].every((id) => IMPORTEES.includes(id)) && changees.size === 4,
      [...changees.keys()].filter((id) => !IMPORTEES.includes(id)).join(", ") || `${changees.size} compagnies`);
    check("48 cartes sur 1 560 changent (appariées par compagnie)", cartes === 48 && total === 1560, `${cartes} / ${total}`);
    check("40 canaux « à confirmer » → accepté sous conditions, 32 → refusé, et RIEN d'autre",
      transitions.get("confirmation_required→accepted_with_conditions") === 40 && transitions.get("confirmation_required→denied") === 32 && transitions.size === 2,
      JSON.stringify([...transitions]));
    check("AUCUN canal ne va vers `allowed`, AUCUN verdict ne bouge (52 conditional, 20 unknown)",
      [...transitions.keys()].every((t) => !t.endsWith("→allowed")) && verdicts.get("conditional→conditional") === 52 && verdicts.get("unknown→unknown") === 20 && verdicts.size === 2,
      JSON.stringify([...verdicts]));
