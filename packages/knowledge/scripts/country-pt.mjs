#!/usr/bin/env node
/* Ajoute une langue à une fiche pays sans réécrire le fichier.
 *
 *   node packages/knowledge/scripts/country-pt.mjs dump   br            > /tmp/br.json
 *   node packages/knowledge/scripts/country-pt.mjs inject br /tmp/br-pt.json
 *
...
if (!mode || !code) {
  console.error("usage : country-pt.mjs dump|inject <code-pays> [fichier.json]");
  process.exit(2);
}
