#!/usr/bin/env node
/**
 * Reproduction du dossier T0-B3-h — une seule commande.
 *
 *   npm run mesure:t0b3h
 *
 * Ce dossier ne lit que des sources et des tables de traduction : ni build, ni moteur, ni réseau.
 * Il se rejoue en une seconde, ce qui est la raison pour laquelle il peut être rejoué souvent.
 *
...
const ARTEFACTS = ["README.md", "ce-que-le-site-dit-en-portugais.json"];
const SOURCES = ["outils/mesurer.mjs", "outils/reproduire.mjs"];

const sha256 = (b) => createHash("sha256").update(b).digest("hex");
const echouer = (m) => { process.stderr.write(`[mesure:t0b3h] ÉCHEC — ${m}\n`); process.exit(1); };
const dire = (m) => process.stdout.write(`[mesure:t0b3h] ${m}\n`);
const arbreSale = () =>
...
{
  const parasite = `.mesure-t0b3h-contre-epreuve-${process.pid}`;
...
const CONTRE_EPREUVES = [
  { code: "es", attendu: "aucun site du site entier ne sert l'anglais en espagnol" },
  { code: "pt", attendu: "l'inventaire portugais est celui du sceau" },
  { code: "clef", attendu: "aucune clé littérale de `t()` n'est incomplète" },
...
dire("dossier T0-B3-h reproductible.");
