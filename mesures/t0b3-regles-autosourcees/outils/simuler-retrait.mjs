 ...
 *   Il répond à : « quand cette règle mord, que change-t-elle, et est-elle DOMINANTE — c'est-à-dire
 *   seule responsable du refus — ou redondante avec une autre ? »
 *
 * Une règle dont le retrait ne change RIEN sur son propre témoin est redondante : une autre règle
 * produit déjà le même refus. Une règle dont le retrait rouvre le canal est dominante : la retirer
 * publierait une acceptation que plus rien ne soutient. C'est exactement le risque nommé en
 * contre-revue — « retirer une règle auto-citée peut rouvrir silencieusement un canal ».
 */
...
for (const r of autoCitees) {
  ...
  if (t.req) {
    ...
    /* SE DÉCLENCHE-T-ELLE VRAIMENT ? Question distincte de « son retrait change-t-il quelque
       chose » : une règle peut mordre sans rien changer, si une autre dit déjà la même chose.
       Le témoin est construit POUR satisfaire ses conditions, mais l'affirmer sans le vérifier
       serait croire mon propre solveur sur parole. On le fait donc trancher par le moteur :
       une base réduite à CETTE SEULE règle, comparée à une base sans aucune règle. Si l'état
       change, c'est qu'elle s'est déclenchée — `evalPredicate` n'étant pas exporté, c'est la
       seule mesure possible sans réimplémenter la grammaire, donc sans créer une deuxième
       vérité à côté du moteur. */
    const kbSeule = normalize({ ...rawKB, rules: [r] });
    const kbVide = normalize({ ...rawKB, rules: [] });
    const cmp = (rapport) => (idEntite ? etatCompagnie(rapport, idEntite) : canonique(rapport).verdict);
    const declenchee = cmp(runFinder(kbSeule, t.req)) !== cmp(runFinder(kbVide, t.req));
    ...
    temoin = {
      ...
      entite_avant: idEntite ? etatCompagnie(avant, idEntite) : null,
      entite_apres: idEntite ? etatCompagnie(apres, idEntite) : null,
      verdict_avant: avantC.verdict, verdict_apres: apresC.verdict,
      score_avant: avantC.score, score_apres: apresC.score,
      /* DOMINANTE : sur son propre témoin, la retirer change l'état de son entité. Redondante :
         le témoin la déclenche mais l'état ne bouge pas — une autre règle dit déjà la même chose. */
      dominante: change && (!idEntite || etatCompagnie(avant, idEntite) !== etatCompagnie(apres, idEntite)),
    };
  }
...
const doc = {
  ...
  synthese: {
    ...
    temoins_ou_la_regle_se_declenche: temoinsConstruits.filter((x) => x.temoin.declenchee).length,
    temoins_ou_elle_NE_se_declenche_PAS: temoinsConstruits.filter((x) => !x.temoin.declenchee).length,
    dominantes_sur_leur_temoin: temoinsConstruits.filter((x) => x.temoin.dominante).length,
    redondantes_sur_leur_temoin: temoinsConstruits.filter((x) => !x.temoin.dominante).length,
  },
  regles: resultats,
};
