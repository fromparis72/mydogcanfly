#!/usr/bin/env node
/**
 * Reproduction du dossier T0-B3-f — une seule commande.
 *
 *   npm run mesure:t0b3f
 *
 * Ce dossier ne lit que le référentiel : ni build, ni moteur, ni réseau. Il se rejoue en une
 * seconde, ce qui est la raison pour laquelle il peut être rejoué souvent.
 *
...
const ARTEFACTS = ["README.md", "ce-que-les-regles-affirment.json"];
const SOURCES = ["outils/mesurer.mjs", "outils/reproduire.mjs"];

const sha256 = (b) => createHash("sha256").update(b).digest("hex");
const echouer = (m) => { process.stderr.write(`[mesure:t0b3f] ÉCHEC — ${m}\n`); process.exit(1); };
const dire = (m) => process.stdout.write(`[mesure:t0b3f] ${m}\n`);
const arbreSale = () =>
...
{
  const parasite = `.mesure-t0b3f-contre-epreuve-${process.pid}`;
...
const CONTRE_EPREUVES = [
  { code: "delai", attendu: "39 concordent, 14 divergent" },
  { code: "permissif", attendu: "le moteur planifie MOINS que sa propre phrase" },
  { code: "traduction", attendu: "en espagnol pour 48" },
  { code: "source", attendu: "portent une source dont l'URL se résout" },
];
...
dire("dossier T0-B3-f reproductible.");
