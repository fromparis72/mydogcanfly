 ...
 *   controle : non_controlee | sans_reference | inchangee | potentiellement_modifiee
...
if (selection.length > 0 && joignables === 0) {
  refus(`0 URL joignable sur ${selection.length} sélectionnée(s) — l'outil ne peut rien contrôler : c'est la signature d'une panne, pas un état des sources`);
}
...
const etatControle = (url) => {
  ...
  return r.empreinte_corps === ref.empreinte_corps
    ? { controle: "inchangee", observe: r }
    : { controle: "potentiellement_modifiee", observe: r, reference: { empreinte_corps: ref.empreinte_corps, capturee_le: ref.capturee_le } };
};
...
const enFile = (l) => l.echeance !== "a_jour"
  || l.controle === "potentiellement_modifiee" || l.controle === "inaccessible" || l.controle === "sans_reference";
...
const rapport = {
  ...
  references: { total: references.references.length },
  selection: { urls: selection.length, urgentes: urgent.size, joignables },
...
md.push(`Sélection : ${selection.length} URL (tranche ${rapport.tranche_du_run}/${N_TRANCHES}, semaine continue ${rapport.semaine_continue}) · ${joignables} joignable(s).`);
...
process.stdout.write(`[fraicheur] ${rapport.registre.entrees} sources · ${selection.length} URL contrôlées (${joignables} joignables) · ` +
  `file de travail : ${file.length} · rapport : ${SORTIE}/rapport-${DATE}.json\n`);
