# §15.10 — Premier lot de tests de référence (v5 — corrections suite à contre-revue Codex du 10/08/2026, tour 3)

**Baseline métier auditée : `e2b27799de335558afc165ee1763ad4613ca4ed9`. État Git du document : commit indiqué par son propre historique Git. Ne pas interpréter cette baseline comme le HEAD courant de `origin/main`** — un SHA recopié ici deviendrait faux dès le commit suivant ; Git connaît déjà le HEAD réel (`git log`/`git rev-parse origin/main`), ce document ne le republie pas. C'est précisément le problème que corrige DR-09 (métadonnées de version automatiques) — tant qu'il n'est pas livré, tout SHA cité dans ce dossier doit être lu comme « au moment du test », pas comme un repère permanent.

**Correction importante par rapport à la v2 de ce document** : la v2 affirmait que le Worker de production servait un code antérieur au 10/08 09:58 (« antérieur à la Phase 1 »), en se fondant sur l'absence de la règle Melbourne lors de plusieurs tests répétés. **Ce diagnostic était faux.** Après le redéploiement de production effectué par Philippe (`npx wrangler deploy --env production`, version `feb7b25d`, confirmée à 100 % du trafic par `wrangler deployments list` — aucun rollout progressif en cause), un nouveau test de Melbourne avec un paramètre de contournement de cache (`&cachebust=...`) a immédiatement montré le résultat correct : **une réponse ancienne a été servie par une couche de cache non encore localisée pour cette URL exacte, pas un Worker resté sur un ancien code.**
**Correction supplémentaire (contre-revue Codex, tour 3)** : la v4 attribuait cette couche au « cache d'edge Cloudflare » — non démontré. Le retest HTTP brut de Codex n'a observé ni `CF-Cache-Status`, ni `Age`, ni `Cache-Control` dans les réponses `/v1/finder`/`/v1/health` ; le Worker V2 ne fixe pas non plus `Cache-Control: no-store`. Il est donc exact de dire qu'une couche de cache a servi une réponse ancienne, mais pas de préciser laquelle sans vérification directe des Cache Rules Cloudflare (demandée à Philippe, voir DR-09) ou sans en-têtes probants. De même, affirmer que « le Worker reflète bien `origin/main` » est trop fort sans `git_sha` déployé ni preuve de la configuration de cache — on peut seulement affirmer que **les comportements testés de Phase 1/2 sont actuellement observés en production**. Toutes les requêtes de test faites APRÈS un déploiement doivent désormais inclure un paramètre variable ET l'enregistrement des en-têtes HTTP bruts — leçon retenue pour la suite des vérifications de ce dossier.

## Distinction utilisée

- **LOCAL_GIT** : comportement du code à la baseline métier `e2b2779`, vérifié par exécution directe du moteur dans un worktree propre.
- **PREVIEW** : toujours NON VÉRIFIABLE — aucune URL de preview Worker confirmée à ce jour ; la preview Pages (`mydogcanfly-v2-preview`, `--branch=main`) est d'un pipeline distinct et non encore clarifiée (voir document 14).
