...
**Réponse confirmée le 10/08/2026** : la page d'accueil `mydogcanfly.com` est le V2 Astro, indexable (pas de balise noindex), avec les métadonnées SEO d'un site en production réelle. Ce n'est plus une hypothèse. Ce qui reste ouvert : le Worker qui sert `/v1/*` en production est une version ancienne du code (antérieure au 10/08 09:48 — voir constat transversal du document 10), donc production ≠ `origin/main` même si le V2 est bien la stack active.
...
## DR-09 — NOUVEAU : dérive entre le dépôt Git et l'artefact réellement déployé
**Constat** : aucun mécanisme du dépôt ne garantit qu'un déploiement correspond à un SHA identifiable. `/v1/health` ne renvoie que `{"ok":true,"service":"mydogcanfly-api","version":"v1"}` — rien qui permette de savoir, sans test manuel indirect comme celui fait aujourd'hui, quelle version de code tourne réellement.
**Décision nécessaire** : Philippe valide-t-il l'ajout de métadonnées de version (`git_sha`, `built_at`, `environment`) injectées au build plutôt que renseignées à la main, à `/v1/health` ? C'est un changement mineur en surface mais qui conditionne la fiabilité de tout futur audit de ce type.
